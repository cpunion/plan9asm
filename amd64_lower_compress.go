...
func (c *amd64Ctx) lowerPackedCompress(op Op, ins Instr) (ok bool, terminated bool, err error) {
	...
	if destination.Kind != OpReg {
		pointer, pointerType, intrinsicSuffix, err := c.packedCompressMemoryPointer(destination)
		if err != nil {
			return true, false, err
		}
...
func (c *amd64Ctx) packedCompressMemoryPointer(destination Operand) (ptr, ptrType, intrinsicSuffix string, err error) {
	...
	case OpMem:
		...
		default:
			return "", "", "", fmt.Errorf("unsupported compress-store pointer type %s", ptrType)
		}
	case OpSym:
	...
	case OpFP:
		...
		return "", "", "", fmt.Errorf("%s %s FP destination has no mutable slot", c.goarch, destination.String())
	default:
		return "", "", "", fmt.Errorf("%s expected a compress-store memory destination, got %s", c.goarch, destination.String())
	}
}
