...
func TestTranslateARM64ScalarPrecisionConversionCompleteGoAssemblerForms(t *testing.T) {
	// Go 1.27 gives FCVTSD and FCVTDS the same one-F-source/one-F-destination
	// AFCVTSD optab shape; their mnemonic letters specify source then result.
...
TEXT scalarprecisionconversionforms(SB),NOSPLIT,$0-0
	FCVTSD F0, F31
	FCVTDS F30, F1
	RET
`)
		...
		for _, want := range []string{"fpext float", "fptrunc double"} {
...
func TestTranslateARM64ScalarPrecisionConversionRejectsFormsOutsideGoOptab(t *testing.T) {
	for _, instruction := range []string{
		...
		if _, err := Translate(file, Options{
			...
			t.Fatalf("Translate accepted %q outside Go 1.27's AFCVTSD optab", instruction)
		}
	}
}
