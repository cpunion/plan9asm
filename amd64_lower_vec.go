...
func (c *amd64Ctx) lowerVec(op Op, ins Instr) (ok bool, terminated bool, err error) {
	...
	if ok, term, err := c.lowerPackedPopcount(rawOp, ins); ok {
		return ok, term, err
	}
	...
	if _, ok := amd64GatherPrefetchSpecs[op]; ok {
		if rawOp != op {
			return true, false, fmt.Errorf("amd64 %s does not accept instruction suffixes: %q", op, ins.Raw)
		}
		return true, false, c.lowerGather(op, ins)
	...
	case "MOVOU", "MOVOA", "MOVUPS", "MOVAPS", "MOVO", "MOVQ", "VMOVQ", "MOVL", "MOVD",
		...
		"VPCMPEQB", "VPCMPGTB", "VPMOVMSKB", "VZEROUPPER", "VZEROALL", "VPBROADCASTB", "VPBROADCASTD", "VPBROADCASTQ", "VPAND", "VPXOR", "VPOR", "VPADDD", "VPADDQ", "VPSUBB", "VPTEST",
		"VPCLMULQDQ", "VEXTRACTF32X4",
		"VPERMB", "VGF2P8AFFINEQB", "VPERMI2B", "VPCMPUQ",
		"VBROADCASTI128", "VBROADCASTF32X2", "VBROADCASTSD",
		...
		"PINSRQ", "PINSRD", "PINSRB", "PINSRW", "PALIGNR", "PUNPCKLBW", "PUNPCKHBW", "PUNPCKLLQ", "PUNPCKHLQ", "PUNPCKLQDQ", "PUNPCKHQDQ", "PSHUFL", "PSHUFD", "SHUFPS",
		"PBLENDW", "PADDQ",
	...
	case "PTEST":
		if len(ins.Args) != 2 || ins.Args[1].Kind != OpReg {
			return true, false, fmt.Errorf("amd64 PTEST expects Xsrc, Xdst: %q", ins.Raw)
		}
		src, err := c.loadXVecOperand(ins.Args[0])
		if err != nil {
			return true, false, err
		...
		zero := c.newTmp()
		fmt.Fprintf(c.b, "  %%%s = icmp eq i128 %%%s, 0\n", zero, andBits)
		fmt.Fprintf(c.b, "  store i1 %%%s, ptr %s\n", zero, c.flagsZSlot)
		notDst := c.newTmp()
		fmt.Fprintf(c.b, "  %%%s = xor <16 x i8> %s, %s\n", notDst, dst, llvmAllOnesI8Vec(16))
		andNot := c.newTmp()
		fmt.Fprintf(c.b, "  %%%s = and <16 x i8> %%%s, %s\n", andNot, notDst, src)
		andNotBits := c.newTmp()
		fmt.Fprintf(c.b, "  %%%s = bitcast <16 x i8> %%%s to i128\n", andNotBits, andNot)
