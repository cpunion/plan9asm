...
func (c *armCtx) lowerRawWord(ins Instr) (bool, error) {
	...
	// VMRS APSR_nzcv,FPSCR transfers the preceding VFP comparison flags. The
	// comparison decoder already materializes those flags in SSA.
	if word&0x0fffffff == 0x0ef1fa10 {
		if !c.flagsWritten {
			return true, fmt.Errorf("%w: raw VMRS has no preceding modeled VFP flag write", ErrProbeNeedsContext)
		}
		return true, nil
	}
