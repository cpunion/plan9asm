...
func (c *amd64Ctx) lowerPackedIntegerCompare(op Op, ins Instr) (ok bool, terminated bool, err error) {
	...
	case "VPCMPEQB", "VPCMPGTB":
		laneBits = 8
	case "VPCMPEQW", "VPCMPGTW":
		laneBits = 16
	case "VPCMPEQD", "VPCMPGTD":
		laneBits = 32
	case "VPCMPEQQ", "VPCMPGTQ":
		laneBits = 64
	default:
	...
	if isAMD64XReg(dst) || isAMD64YReg(dst) {
		...
		if !c.isGoVEXVectorRegister(ins.Args[0], byteWidth, true) ||
...
func (c *amd64Ctx) comparePackedIntegerLanes(baseOp string, laneBits, byteWidth int, first, second Operand, broadcast bool) (string, error) {
	firstValue, err := c.loadPackedCompareLanes(first, byteWidth, laneBits, broadcast)
	if err != nil {
		return "", err
	}
	...
	predicate := "eq"
	if strings.HasPrefix(baseOp, "VPCMPGT") {
		predicate = "sgt"
	}
	lanes := byteWidth * 8 / laneBits
	compared := c.newTmp()
...
func (c *amd64Ctx) emitPackedIntegerVectorCompare(baseOp string, laneBits, byteWidth int, first, second Operand, dst Reg) error {
	...
	case 32:
		return c.storeY(dst, "%"+bytesValue)
	default:
		return fmt.Errorf("Go 1.27 vector-result packed compare has no %d-byte form", byteWidth)
	}
}
...
func (c *amd64Ctx) emitPackedIntegerMaskCompare(baseOp string, laneBits, byteWidth int, first, second Operand, dst Reg, writeMask string, broadcast bool) error {
	compared, err := c.comparePackedIntegerLanes(baseOp, laneBits, byteWidth, first, second, broadcast)
	if err != nil {
		return err
	}
	lanes := byteWidth * 8 / laneBits
