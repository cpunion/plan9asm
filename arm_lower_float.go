...
func armFloatMoveModifiers(ins Instr) (postIndex, writeback, up bool, err error) {
	for _, suffix := range armInstructionSuffixes(ins) {
		switch suffix {
		case "", "AL":
		case "P":
		...
		default:
			if !armCondCodes[suffix] {
				return false, false, false, fmt.Errorf("arm %s suffix %q is absent from the Go 1.27 MOVF/MOVD optab: %q", ins.Op, suffix, ins.Raw)
			}
		}
	}
...
func (c *armCtx) lowerARMFloatMove(op, cond string, ins Instr) (ok bool, terminated bool, err error) {
	...
	postIndex, writeback, up, err := armFloatMoveModifiers(ins)
	if err != nil {
